...
function AdultCategoriesPageContent() {
  const router = useRouter()
  const searchParams = useSearchParams()
  const adultId = searchParams.get("adultId")

  const [adult, setAdult] = useState<Adult | null>(null)
  const [categories, setCategories] = useState<AdultCategory[]>([])
  const [items, setItems] = useState<Record<number, AdultExpenseItem[]>>({})
  const [isItemDialogOpen, setIsItemDialogOpen] = useState(false)
  const [editingItem, setEditingItem] = useState<AdultExpenseItem | null>(null)
  const [currentCategoryId, setCurrentCategoryId] = useState<number | null>(null)
  const [deleteItem, setDeleteItem] = useState<AdultExpenseItem | null>(null)
  ...
  useEffect(() => {
    if (adultId) {
      loadData(Number.parseInt(adultId))
    }
  }, [adultId])
  useReloadOnSync(() => {
    if (adultId) void loadData(Number.parseInt(adultId))
  })
  ...
  async function loadData(id: number) {
    ...
    const itemsData: Record<number, AdultExpenseItem[]> = {}
    for (const category of categoriesData) {
      if (category.id) {
        const categoryItems = await db.adultItems.where("categoryId").equals(category.id).toArray()
  ...
  function openEditItemDialog(item: AdultExpenseItem, categoryId: number) {
    setCurrentCategoryId(categoryId)
    setEditingItem(item)
    setItemFormData({
      name: item.name,
  ...
  async function handleItemSubmit(e: React.FormEvent) {
    ...
    if (editingItem?.id) {
      await db.adultItems.update(editingItem.id, itemData)
    } else {
      await db.adultItems.add(itemData)
    }
  ...
  async function confirmDeleteItem() {
    ...
    await db.adultItems.delete(deleteItem.id)
  ...
  async function handleMiscPercentageUpdate(categoryId: number, percentage: number) {
    await db.adultCategories.update(categoryId, { percentageValue: percentage })
    if (adultId) {
      await loadData(Number.parseInt(adultId))
    }
  ...
  async function handleInlineCostUpdate(item: AdultExpenseItem, newCost: number) {
    if (!item.id) return
    const total = calculateAnnualCost(newCost, item.frequency, item.quantity)
    await db.adultItems.update(item.id, { cost: newCost, total })
    if (adultId) {
      await loadData(Number.parseInt(adultId))
    }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-primary/5 via-secondary/5 to-accent/5">
      <div className="mx-auto max-w-4xl px-4 py-8">
        ...
        {/* Header */}
        <div className="mb-6">
          <Button variant="ghost" onClick={() => router.push("/adults")} className="mb-4 gap-2">
            <ChevronLeft className="h-4 w-4" />
            Back to Adults
          </Button>
          <h1 className="mb-2 font-serif text-3xl font-bold text-primary">{adult.name}&apos;s Budget</h1>
...
export default function AdultCategoriesPage() {
  return (
    <Suspense
      fallback={
        <div className="flex min-h-screen items-center justify-center">
