...
function CategoriesPageContent() {
  ...
  const [items, setItems] = useState<Record<number, ExpenseItem[]>>({})
  ...
  useEffect(() => {
    if (childId) {
      loadData(Number.parseInt(childId))
    }
  }, [childId])
  useReloadOnSync(() => {
    if (childId) void loadData(Number.parseInt(childId))
  })
  ...
  async function loadData(id: number) {
    ...
    const itemsData: Record<number, ExpenseItem[]> = {}
    for (const category of categoriesData) {
      if (category.id) {
        const categoryItems = await db.items.where("categoryId").equals(category.id).toArray()
  ...
  function openEditItemDialog(item: ExpenseItem, categoryId: number) {
    setCurrentCategoryId(categoryId)
    setEditingItem(item)
    setItemFormData({
      name: item.name,
  ...
  async function handleMiscPercentageUpdate(categoryId: number, percentage: number) {
    await db.categories.update(categoryId, { percentageValue: percentage })
    if (childId) {
      await loadData(Number.parseInt(childId))
    }
  ...
  async function handleInlineCostUpdate(item: ExpenseItem, newCost: number) {
    if (!item.id) return
    const total = calculateAnnualCost(newCost, item.frequency, item.quantity)
    await db.items.update(item.id, { cost: newCost, total })
    if (childId) {
      await loadData(Number.parseInt(childId))
    }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-primary/5 via-secondary/5 to-accent/5">
      <div className="mx-auto max-w-4xl px-4 py-8">
        ...
        {/* Header */}
        <div className="mb-6">
          <Button variant="ghost" onClick={() => router.push("/children")} className="mb-4 gap-2">
            <ChevronLeft className="h-4 w-4" />
            Back to Children
          </Button>
          <h1 className="mb-2 font-serif text-3xl font-bold text-primary">{child.name}'s Budget</h1>
          <p className="text-muted-foreground">
            {child.age} years old • {child.schoolLevel}
          </p>
        </div>
          ...
          <form onSubmit={handleItemSubmit} className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                ...
                {itemFormData.frequency === "term" && "Number of terms (usually 3-4)"}
